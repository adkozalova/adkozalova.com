...
function Item(props) {
  ...
  if (item.link === null || item.link === undefined) {
    return (
        <li>
          <span>{item.date}</span>
          {item.title} {item.location === null || item.location === undefined ? "" : <i className="fa fa-map-marker" aria-hidden="true"/>}&nbsp;{item.location}
        </li>
    )
  } else {
    return (
        <li>
          ...
          <a href={item.link} target={"_blank"} rel="noopener noreferrer">{item.title}</a>
            {item.location === null || item.location === undefined ? "" : <i className="fa fa-map-marker" aria-hidden="true"/>}&nbsp;{item.location}
        </li>
    )
  }
}
